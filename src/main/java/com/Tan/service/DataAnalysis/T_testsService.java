...
public class T_testsService {
    ...
    //独立样本贝叶斯 T 检验
    public Map independentBayesSample(List<InputData> dataList,String rscale)
    {
        RConnection rc=null;//Rserve连接
        Map result=new HashMap();//返回前端的结果
        ...
        try{
            ...
            {
                rc.assign(item.getHead(),item.getData());
                RList rList=rc.eval("BayesFactor::extractBF(BayesFactor::ttestBF(x="+groupVar.getHead()+",y="+item.getHead()+",paired=FALSE,rscale='"+rscale+"'))").asList();
                Map item_res=new HashMap();
                //处理数据
                item_res.put("bf",rList.at("bf").asDouble());// bf值
                item_res.put("error",rList.at("error").asDouble());//error
                //添加到result
                result.put(item.getHead(),item_res);
            }
    ...
    //贝叶斯配对样本 T 检验
    public Map bayesPairedSample(List<InputData> dataList,String rscale)
    {
        RConnection rc=null;//Rserve连接
        Map result=new HashMap();//返回前端的结果
        ...
        try{
            ...
            for(int i=0;i<dataList.size();i+=2)
            {
                ...
                rc.assign(pairA.getHead(),pairA.getData());
                rc.assign(pairB.getHead(),pairB.getData());
                RList rList=rc.eval("BayesFactor::extractBF(BayesFactor::ttestBF(x="+pairA.getHead()+",y="+pairB.getHead()+",paired=TRUE,rscale='"+rscale+"'))").asList();
                Map item_res=new HashMap();
                //处理数据
                item_res.put("bf",rList.at("bf").asDouble());// bf值
                item_res.put("error",rList.at("error").asDouble());//error值
                //添加到result
                result.put(pairA.getHead()+"~"+pairB.getHead(),item_res);
            }
    ...
    //贝叶斯单样本 T 检验
    public Map bayesOneSample(List<InputData> dataList,String rscale,double mu)
    {
        RConnection rc=null;//Rserve连接
        Map result=new HashMap();//返回前端的结果
        ...
        try{
            ...
            for(InputData item:dataList)
            {
                //R处理
                rc.assign(item.getHead(),item.getData());
                RList rList=rc.eval("BayesFactor::extractBF(BayesFactor::ttestBF(x="+item.getHead()+",paired=FALSE,mu="+mu+",rscale='"+rscale+"'))").asList();
                Map item_res=new HashMap();
                //处理数据
                item_res.put("bf",rList.at("bf").asDouble());// bf值
                item_res.put("error",rList.at("error").asDouble());//error
